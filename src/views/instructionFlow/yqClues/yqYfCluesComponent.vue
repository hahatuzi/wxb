    ...
    export default {
        ...
        methods:{
            handleCurrentChange(val){
                ...
                this.query();
                this.startNum = this.yqYfCluesList.length > 0? ((this.currentPage -1)* this.pageSize +1):0;
                this.endNum = this.yqYfCluesList.length > 0 ?(this.startNum + this.yqYfCluesList.length -1):0;
            },
            ...
            info(row){
              ...
              this.$router.push({path:'/instructionFlow/yqYfClueInfo',query:{operateType:'1',id:row.infoid}});
            },
            addInfo(){
                this.$router.push({path:'/instructionFlow/yqYfClueInfo',query:{operateType:'0'}});
            },
            modify(row){
                this.$router.push({path:'/instructionFlow/yqYfClueInfo',query:{operateType:'2',id:row.infoid}});
            },
            sousuo(){
                this.currentPage = 1;
                this.query();
            ...
            query(){
                ...
                let url = `/yqgz/inform/aloneUnitClue/queryClueListBS?pageNum=${this.currentPage}&pageSize=${this.pageSize}&unit=yf`
                getList(url, yqgzTdinfo).then(result =>{
                    console.log(result);
                    this.loading = false;
                    this.yqYfCluesList = result.rows;
