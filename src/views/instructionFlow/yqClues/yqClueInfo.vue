...
<script>
    ...
    import {addClues,updateClues,addOrUpdateCluesSign, queryClue} from '@/api/yqClues/index'
    ...
    export default {
        ...
        methods:{
            returnToList(){
              this.$router.push('/instructionFlow/yqClue');
            },
            query(){
              var _this = this;
              queryClue(_this.$route.query.id).then(result => {
                _this.form = result.data;
                _this.fileList = result.data.fileList;
                this.editor.txt.html(result.data.infocontent);
                if(_this.$route.query.operateType == 2){
                  _this.fileList = _this.form.fileList.slice(0);
                  // _this.fileIdList = [];
                  for(let i in _this.fileList){
                    _this.fileList[i].name = _this.fileList[i].file_name;
                    _this.fileChange(_this.fileList[i]);
                    _this.fileIdList.push(_this.fileList[i].file_id);
                  }
                }
              })
            },
            ...
            report(val){
              ...
              //       _this.$router.push('/yqInforComponent/yqInfo_yqClues');
              //     })
              //   })
