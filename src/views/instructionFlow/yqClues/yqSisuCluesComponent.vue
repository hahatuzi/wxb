...
<script>
    import { getList, deleteClue } from '@/api/yg/aloneUtil'
    const typeOptions =  ['舆情线索','情况通报','工作动态','分析研究','约稿','其他'];

    export default {
        name: "yqSisuCluesComponent",
        data(){
          return {
            searchForm:{
              title:'',
        ...
        methods:{
            handleCurrentChange(val){
                ...
                this.query();
                this.startNum = this.yqSisuCluesList.length > 0? ((this.currentPage -1)* this.pageSize +1):0;
                this.endNum = this.yqSisuCluesList.length > 0 ?(this.startNum + this.yqSisuCluesList.length -1):0;
            },
            ...
            info(row){
              ...
              this.$router.push({path:'/instructionFlow/yqSisuClueInfo',query:{operateType:'1',id:row.infoid}});
            },
            addInfo(){
                this.$router.push({path:'/instructionFlow/yqSisuClueInfo',query:{operateType:'0'}});
            },
            modify(row){
                this.$router.push({path:'/instructionFlow/yqSisuClueInfo',query:{operateType:'2',id:row.infoid}});
            },
            sousuo(){
                this.currentPage = 1;
                this.query();
            ...
            query(){
                ...
                var end = '';
                if(this.searchForm.startDate) {
                    start = new Date(this.searchForm.startDate);
                    start = start.getFullYear() + '-' + (start.getMonth() + 1) + '-' + start.getDate();
                }
                if(this.searchForm.endDate){
                    end = new Date(this.searchForm.endDate);
                ...
                var yqgzTdinfo = {
                    ...
                    createtime: "",
                    disposalRemark: "",
                    disposalSituation: this.searchForm.dispose,
                    endTime: end,
                    fileIdList: [],
                    fileList: [{}],
                    groupid: "",
                    iinfolink: "",
                    iname: this.searchForm.title,
                    infocontent: "",
                    infoid: "",
                    istate: this.searchForm.adopt,
                    remark: "",
                    reportdate: "",
                    reportunit: "",
                    reportunitname: "",
                    ...
                    resultusernmae: "",
                    startTime: start,
                    submissionList: this.searchForm.bsType,
                    submissionType: ""
                };
                let url = `/yqgz/inform/aloneUnitClue/queryClueListBS?pageNum=${this.currentPage}&pageSize=${this.pageSize}`
                getList(url, yqgzTdinfo).then(result =>{
                    console.log(result);
                    this.loading = false;
                    this.yqSisuCluesList = result.rows;
